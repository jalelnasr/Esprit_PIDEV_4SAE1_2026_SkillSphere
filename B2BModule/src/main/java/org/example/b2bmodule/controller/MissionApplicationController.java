...
public class MissionApplicationController {
     ...
     */
    @PostMapping
    public ResponseEntity<MissionApplicationResponse> apply(@RequestBody org.example.b2bmodule.dto.MissionApplicationRequest request) {
        log.info("POST /api/b2b/mission-applications - Apply for mission");
        MissionApplicationResponse response = missionApplicationService.apply(request);
    ...
    /**
     * Get top matches for a mission (sorted by match score)
     */
    ...
    public ResponseEntity<List<MissionApplicationResponse>> getTopMatches(
        ...
        // Get all applications and return top ones (can be enhanced with sorting by matchScore)
        List<MissionApplicationResponse> applications = missionApplicationService.findByMission(missionId);
        List<MissionApplicationResponse> topMatches = applications.stream()
                .limit(limit)
                .toList();
