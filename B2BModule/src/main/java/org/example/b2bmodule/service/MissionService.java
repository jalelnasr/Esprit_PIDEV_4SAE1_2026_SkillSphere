...
public class MissionService {
    ...
    public List<MissionResponse> findAll() { return missionRepository.findAll().stream().map(this::toResponse).toList(); }
    public MissionResponse findById(Long id) { return toResponse(missionRepository.findById(id).orElseThrow(() -> new RuntimeException("Mission not found"))); }
    public List<MissionResponse> findByCompany(Long companyId) { return missionRepository.findByCompanyId(companyId).stream().map(this::toResponse).toList(); }
    public List<MissionResponse> findOpen() { return missionRepository.findByStatus(Mission.MissionStatus.OPEN).stream().map(this::toResponse).toList(); }
