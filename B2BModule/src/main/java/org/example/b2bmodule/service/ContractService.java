...
public class ContractService {
    ...
    private ContractResponse toResponse(Contract c) {
        return new ContractResponse(c.getId(), c.getContractNumber(), c.getMission() != null ? c.getMission().getId() : null, c.getMission() != null ? c.getMission().getTitle() : null, c.getCandidate().getId(), c.getCompany().getId(), c.getCompany().getName(), c.getTotalAmount(), c.getStartDate(), c.getEndDate(), c.getContractUrl(), c.getStatus().name(), c.getSignedAt(), c.getSignatureData());
    }
    ...
    public ContractResponse create(ContractRequest req) {
        ...
        // Vérifier si un contrat existe déjà pour cette mission ET ce candidat
        var existingContract = contractRepository.findByMissionIdAndCandidateId(req.missionId(), req.candidateId());
        if (existingContract.isPresent()) {
            // Retourner le contrat existant au lieu de créer un nouveau
            return toResponse(existingContract.get());
        }

        // Créer un nouveau contrat (plusieurs contrats par mission sont maintenant autorisés)
    ...
    public ContractResponse signWithSignature(Long id, String signatureData) {
        Contract c = contractRepository.findById(id).orElseThrow(() -> new RuntimeException("Contract not found"));
        c.setStatus(Contract.ContractStatus.SIGNED);
        c.setSignedAt(Instant.now());
        c.setSignatureData(signatureData);
        return toResponse(contractRepository.save(c));
    }
