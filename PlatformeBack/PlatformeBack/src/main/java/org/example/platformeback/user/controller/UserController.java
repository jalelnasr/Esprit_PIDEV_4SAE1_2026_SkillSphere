...
public class UserController {
    ...
    // ======================
    //  ⭐ NEW: Public endpoint for role verification
    //  Used by Professional Events microservice
    // ======================

    /**
     * Get user role by ID
     * Used by Professional Events to verify if user can register to competitions
     * Only APPRENANT can register, FORMATEUR cannot
     */
    @GetMapping("/{userId}/role")
    public String getUserRole(@PathVariable Long userId) {
        UserResponse user = userService.adminGetUser(userId);
        return user.role().name(); // Returns "APPRENANT" or "FORMATEUR" or "ADMIN"
    }
}