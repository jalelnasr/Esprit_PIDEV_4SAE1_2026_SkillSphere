...
@Component({
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <div class="email-field-wrap">
                <input type="email" [(ngModel)]="emailForm.candidateEmail" placeholder="Loading email..." class="form-control"
                       [readonly]="emailAutoFilled" [class.auto-filled]="emailAutoFilled">
                <span *ngIf="loadingEmail" class="email-loading">⏳</span>
                <span *ngIf="emailAutoFilled" class="email-check">✅</span>
              </div>
              <small *ngIf="emailAutoFilled" class="email-hint">Auto-filled from user profile</small>
            </div>
            <div class="form-group">
  ...
  styles: [`
    ...
    .form-control.auto-filled { background: #f0fdf4; border-color: #86efac; color: #16a34a; font-weight: 600; }
    :root.dark-mode .form-control.auto-filled { background: rgba(22,163,106,0.1); border-color: rgba(34,197,94,0.4); color: #4ade80; }
    .email-field-wrap { position: relative; }
    .email-field-wrap input { padding-right: 40px; }
    .email-loading, .email-check { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); font-size: 16px; }
    .email-hint { display: block; margin-top: 4px; font-size: 12px; color: #16a34a; font-weight: 500; }
    :root.dark-mode .email-hint { color: #4ade80; }
...
export class JobOfferFormComponent implements OnInit {
  ...
  loadCandidateEmail(candidateId: number) {
    ...
    this.emailAutoFilled = false;
    // candidateId is also the userId
    this.http.get<any>(`/api/users/${candidateId}`).subscribe({
      next: (user: any) => {
        if (user?.email) {
      ...
      error: () => {
        this.loadingEmail = false;
        // User will enter email manually
      }
    });
  }
