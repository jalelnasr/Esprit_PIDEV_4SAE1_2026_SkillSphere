  ...
  onLogin(): void {
    ...
    this.authService.login(email, password).subscribe({
      next: (res) => {
        ...
        else if (res.role === 'FORMATEUR') this.router.navigate(['/dashboard']); // or trainer route
        else if (res.role === 'RH_ENTREPRISE') this.router.navigate(['/dashboard']); // or enterprise route
        else this.router.navigate(['/dashboard']); // learner
      },
      error: () => {
        this.isLoading = false;
